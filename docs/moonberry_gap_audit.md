...
Additional 2026-08-07 update: scene possession movement is now split into a speed component (fully affected by speed bonuses and reductions) and a fixed base allowance of 10 that ignores speed bonuses but is scaled down by percentage speed reductions (`AddPercent` below 0 or `SetPercentOfBase` below 100) sourced from equipment, active buffs, legacy passives, and approved talent passives. `疾行如风` approved talent now grants +100% scene movement speed from `开团` until the character's group enters an active battle or ten world turns have passed since the persisted `TrpgGroup.world_start_turn`; the bonus applies only to the speed component, so the fixed base 10 stays untouched by it. Focused verification passes with `cargo test --lib -j 1 swift_wind`, `cargo test --lib -j 1 possession_movement`, and `cargo test --lib -j 1 character_in_active_encounter`: 3, 4, and 1 passed respectively, 0 failed.
...
Additional 2026-07-18 offline-PC persistence correction: chat-target synchronization no longer treats the presence of private message history as the sole proof that a player character exists. PC-only imports and Moonberry group characters with no chat records now survive the next UI synchronization together with their TRPG group membership; established empty-history player placeholders also remain compatible. A target backed by a definite QQ group-message history is still rejected as a player character, and group members with neither a durable PC nor any player-target record are still pruned. Focused targeted-PC import, Moonberry no-chat migration, and offline/group-target synchronization verification passes: 3 passed, 0 failed. `cargo check -j 1 --quiet` passes. Full library verification passes with `cargo test --lib -j 1 --quiet`: 472 passed, 1 ignored live API test, 0 failed.

Additional 2026-07-18 chat-list target-kind correction: targeted chat-list imports now persist each history-free target's exported private/group kind instead of discarding it with the intentionally omitted message bodies. Imported private targets therefore synchronize to PCs and enter the selected TRPG group when approved, while imported QQ group targets cannot be misclassified as players. Real persisted or newly ingested NapCat messages remain authoritative and repair stale imported kind metadata; legacy empty message vectors still synchronize to editable unknown-kind chat metadata. Moonberry PC/chat-list imports and acknowledged local private replies also record the durable private kind. Focused import, approval, stale-kind, and empty-history compatibility verification passes: 5 passed, 0 failed. `cargo check -j 1 --quiet` passes. Full library verification passes with `cargo test --lib -j 1 --quiet`: 474 passed, 1 ignored live API test, 0 failed.

Additional 2026-07-18 history-free chat usability correction: chat-list rows, standalone windows, player/group target pools, private broadcasts, onboarding guides, summary scope labels, group-name lookup, and outbound routing now use the canonical persisted target kind when no message body exists. A freshly imported private or group target is immediately visible and sends through the correct NapCat action; its first acknowledged send or onboarding reply creates local history even when no prior message can supply the bot QQ id, using the same zero-id legacy-safe self marker as Moonberry imports. Supplied real messages remain authoritative for pre-persistence summary/preview calculations. Four new history-free UI/onboarding/local-history regressions pass. `cargo check -j 1 --quiet` passes. Full library verification passes with `cargo test --lib -j 1 --quiet`: 478 passed, 1 ignored live API test, 0 failed.

Additional 2026-07-18 battle-backup correction: the separately persisted battle-round store now has a versioned JSON backup/restore surface in the Chinese TRPG settings UI. Exact replacement import preserves active encounter selection, TRPG group/campaign binding, round clocks, action completion, participant vitals, unit snapshots, shields, delayed effects, talent/combat counters, contributors, skill cooldown clocks, and action logs without risking a silent merge of duplicate live encounters. Import rejects wrong formats and empty identities, repairs duplicate participants, normalizes vitals, and clears a stale active encounter id before persistence. Three focused format/repair/rejection regressions pass. `cargo check -j 1 --quiet` passes. Full library verification passes with `cargo test --lib -j 1 --quiet`: 481 passed, 1 ignored live API test, 0 failed.

Additional 2026-07-18 cross-backup isolation correction: replacing the root NapCat/TRPG manager now persistently quarantines every existing battle encounter before it can synchronize into the newly imported characters. Quarantined encounters expose no combat controls and the state-layer manager-sync guard rejects HP, MP, turn, counter, and cooldown writes even if a future caller bypasses the panel. The Chinese battle UI preserves the old battle data and offers an explicit destructive-state reconnect only when the linked group still exists and its durable campaign ID matches; legacy same-group encounters bind that ID upon confirmation, while missing or mismatched links remain delete-only. Importing an older battle backup remains compatible because the new quarantine field defaults to disabled. Four focused quarantine, write-denial, compatibility, and reconnect-identity regressions pass. `cargo check -j 1 --quiet` passes. Full library verification passes with `cargo test --lib -j 1 --quiet`: 485 passed, 1 ignored live API test, 0 failed.
...
3. Talent support backlog (very low priority; do not schedule ahead of items 4-6):
