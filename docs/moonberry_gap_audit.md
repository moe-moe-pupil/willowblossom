...
Additional 2026-07-16 correction: automatic battle healing now respects the encounter's explicit defeated state. Delayed healing and healing buff ticks expire without reviving a participant whose `alive` flag is false or emitting a false healing log; direct healing skills retain the established ability to restore a zero-HP target, and `希望化身` remains healable while active. Revival therefore requires a directed healing action or an explicit GM/state transition rather than an accidental background effect. Focused verification passes with `cargo test --lib background_healing_does_not_revive_defeated_participants`.
