...
pub(crate) const NORMAL_TALENT_POOL: &[MoonberryTalent] = &[
    ...
    MoonberryTalent {
        name: r#"镜像外衣"#,
        description: r#"当你进入战斗轮时，如果受到致命伤害，你会移除会造成伤害的有害buff并将生命值变成1，然后获得一层镜像外衣，之后会消耗最多15魔法值，每消耗5点魔法值会额外叠加一层镜像外衣，每层镜像外衣能使你隐身一回合,上限4回合，同时脱离战斗轮，如果再次受到或造成伤害则会立刻进入战斗轮并脱离隐身，冷却需要6回合。"#,
    },
    MoonberryTalent {
        name: r#"溃伤"#,
...
pub(crate) const SUPPORT_TALENT_POOL: &[MoonberryTalent] = &[
    ...
    MoonberryTalent {
        name: r#"蝴蝶效应"#,
        description: r#"每一次跑团开始时，你将可以指定一个除自身外的目标，使他获得5%的伤害加成和5%的伤害减免，持续一回合，然后在一回合后转移此效果给你，之后再转移给他，如此反复。如果其中一个目标死亡，效果就会一直存在于另一个目标身上。"#,
    },
    MoonberryTalent {
        name: r#"强化护盾"#,
